...
void TransactionRecord::updateStatus(const CWalletTx &wtx, int chainLockHeight)
{
    ...
    {
        // The IsLockedByDirectSend call is quite expensive, so we only do it when a state change is actually possible.
        if (status.lockedByChainLocks) {
            if (oldLockedByChainLocks != status.lockedByChainLocks) {
                status.lockedByDirectSend = wtx.IsLockedByDirectSend();
            } else {
                status.lockedByDirectSend = false;
            }
        } else if (!status.lockedByDirectSend) {
            status.lockedByDirectSend = wtx.IsLockedByDirectSend();
        }
