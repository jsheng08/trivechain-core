...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    for(const auto& tx : block.vtx)
    {
        if(txDetails)
        {
            ...
            bool fLocked = llmq::quorumDirectSendManager->IsLocked(tx->GetHash());
...
void entryToJSON(UniValue &info, const CTxMemPoolEntry &e)
{
    ...
    info.push_back(Pair("instantlock", llmq::quorumDirectSendManager->IsLocked(tx.GetHash())));
}
...
UniValue getmempoolinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"directsendlocks\": xxxxx,   (numeric) Number of unconfirmed instant send locks\n"
            "}\n"
            "\nExamples:\n"
