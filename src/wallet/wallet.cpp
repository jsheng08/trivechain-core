    ...
    for (const auto& entry : mapWallet) {
        ...
        for (const CTxOut& out : wtx.tx->vout) {
            if (outgoing && IsChange(out)) {
                debit -= out.nValue;
            } else if (IsMine(out) & filter && (depth >= minDepth || (fAddLocked && wtx.IsLockedByDirectSend())) && (!account || *account == GetAccountName(out.scriptPubKey))) {
                balance += out.nValue;
            }
        }
...

    {
        ...
        for (auto pcoin : GetSpendableTXs()) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                bool found = false;
                if(nCoinType == CoinType::ONLY_1000) {
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, CoinType nCoinType) const
{
    ...
    for (unsigned int tryDenom = tryDenomStart; tryDenom < 2; tryDenom++)
    {
        ...
        for (const COutput &output : vCoins)
        {
            ...
            bool fLockedByIS = pcoin->IsLockedByDirectSend();
...
void CWallet::NotifyTransactionLock(const CTransaction &tx, const llmq::CDirectSendLock& islock)
{
    LOCK(cs_wallet);
    ...
    if (mi != mapWallet.end()){
        ...
        std::string strCmd = gArgs.GetArg("-directsendnotify", "");
        if (!strCmd.empty()) {
