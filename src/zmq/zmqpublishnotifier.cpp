...
bool CZMQPublishHashTransactionLockNotifier::NotifyTransactionLock(const CTransaction &transaction, const llmq::CDirectSendLock& islock)
{
    uint256 hash = transaction.GetHash();
...
bool CZMQPublishHashDirectSendDoubleSpendNotifier::NotifyDirectSendDoubleSpendAttempt(const CTransaction &currentTx, const CTransaction &previousTx)
{
    ...
    LogPrint(BCLog::ZMQ, "zmq: Publish hashdirectsenddoublespend %s conflicts against %s\n", currentHash.ToString(), previousHash.ToString());
...
bool CZMQPublishRawTransactionLockNotifier::NotifyTransactionLock(const CTransaction &transaction, const llmq::CDirectSendLock& islock)
{
    uint256 hash = transaction.GetHash();
...
bool CZMQPublishRawTransactionLockSigNotifier::NotifyTransactionLock(const CTransaction &transaction, const llmq::CDirectSendLock& islock)
{
    uint256 hash = transaction.GetHash();
...
{
    LogPrint(BCLog::ZMQ, "zmq: Publish rawdirectsenddoublespend %s conflicts with %s\n", currentTx.GetHash().ToString(), previousTx.GetHash().ToString());
