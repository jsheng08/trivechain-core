...
static int64_t nTimeTrivechainSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    /// TRIVECHAIN: Check superblock start
    ...
    if (sporkManager.IsSporkActive(SPORK_3_DIRECTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(TRIVECHAIN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(TRIVECHAIN): spork is off, skipping transaction locking checks\n");
    }
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(TRIVECHAIN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeTrivechainSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Trivechain specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeTrivechainSpecific * 0.000001);
