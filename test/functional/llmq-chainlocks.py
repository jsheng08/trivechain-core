#!/usr/bin/env python3
# Copyright (c) 2015-2020 The Trivechain Core developers
...
class LLMQChainLocksTest(TrivechainTestFramework):
    def set_test_params(self):
        self.set_trivechain_test_params(6, 5, fast_dip3_enforcement=True)
    ...
    def run_test(self):
        ...
        self.log.info("Enable LLMQ bases DirectSend, which also enables checks for \"safe\" transactions")
        self.nodes[0].spork("SPORK_2_DIRECTSEND_ENABLED", 0)
        self.nodes[0].spork("SPORK_3_DIRECTSEND_BLOCK_FILTERING", 0)
        self.wait_for_sporks_same()
        ...
        time.sleep(1)
        assert(not self.nodes[0].getblock(self.nodes[0].getbestblockhash())["chainlock"])
        self.log.info("Disable LLMQ based DirectSend for a very short time (this never gets propagated to other nodes)")
