...
class BitcoinTestFramework(object):
    ...
    def stop_node(self, i, wait=0):
        """Stop a trivechaind test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple trivechaind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as trivechaind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("TRIVECHAIND", "trivechaind"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0", "-mocktime="+str(GENESISTIME)]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in trivechain.conf
...
class TrivechainTestFramework(BitcoinTestFramework):
    def set_trivechain_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    Sets up some trivechaind binaries:
